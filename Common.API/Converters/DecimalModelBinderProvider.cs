...
namespace Common.API
{
    public class NumberModelBinderProvider : IModelBinderProvider
    {
        public IModelBinder GetBinder(ModelBinderProviderContext context)
        {
            ...
            if (!context.Metadata.IsComplexType && context.Metadata.ModelType == typeof(int))
    ...
    internal class IntBinder : IModelBinder
    {
        public Task BindModelAsync(ModelBindingContext bindingContext)
        {
            var valueProviderResult = bindingContext.ValueProvider.GetValue(bindingContext.ModelName);
            ...
            var parsed = Int32.TryParse(value, out var valueAsInt);

            var result = ModelBindingResult.Success(valueAsInt);
            if (!parsed)
            {
                result = ModelBindingResult.Failed();
                bindingContext.ModelState.AddModelError(bindingContext.ModelName, "Invalid Int");
            }
